...
try {
    ...
    // Slug extraction: prefer ?slug query param (set by .htaccess rewrite),
    // then fall back to REQUEST_URI parsing (PATH_INFO unreliable on shared hosting)
    $path = trim((string)($_GET['slug'] ?? ''), '/');
    if ($path === '') {
